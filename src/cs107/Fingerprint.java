...
public class Fingerprint {
  ...
  public static boolean[] getNeighbours(boolean[][] image, int row, int col) {
      ...
      if(row>=0 && row<image.length && col>=0 && col<image[0].length) {
          ...
          boolean[][] extendedImage = new boolean[image.length + 2][image[0].length + 2];
          ...
          row++;
          col++;
          neighbours[0] = extendedImage[row-1][col];
          neighbours[1] = extendedImage[row-1][col+1];
          neighbours[2] = extendedImage[row][col+1];
          neighbours[3] = extendedImage[row+1][col+1];
          neighbours[4] = extendedImage[row+1][col];
          neighbours[5] = extendedImage[row+1][col-1];
          neighbours[6] = extendedImage[row][col-1];
          neighbours[7] = extendedImage[row-1][col-1];
	      ...
	      case 0:
              for(int i=0; i<extendedImage.length; i++) {
                  for(int j=0; j<extendedImage[0].length; j++) {
                      ...
                      if(extendedImage[i][j] == true &&
                      getNeighbours(image, i, j) != null &&
                      2 <= blackNeighbours(neighbours) && blackNeighbours(neighbours) <= 6 &&
                      transitions(neighbours) == 1 &&
                      (extendedImage[i-1][j] == false || extendedImage[i][j+1] == false || extendedImage[i+1][j] == false) &&
                      (extendedImage[i][j+1] == false || extendedImage[i+1][j] == false || extendedImage[i][j-1] == false)) {
                          extendedImage[i][j] = false;
                      }
                  }
              }
              for(int i=0; i<image.length; i++) {
                  for(int j=0; j<image[0].length; j++) {
                       image[i][j] = extendedImage[i+1][j+1];
                  }
              }
          case 1:
              for(int i=0; i<extendedImage.length; i++) {
                  for(int j=0; j<extendedImage[0].length; j++) {
                      ...
                      if(extendedImage[i][j] == true &&
                              ...
                              transitions(neighbours) == 1 &&
                              (extendedImage[i-1][j] == false || extendedImage[i][j+1] == false || extendedImage[i+1][j] == false) &&
                              (extendedImage[i-1][j] == false || extendedImage[i+1][j] == false || extendedImage[i][j-1] == false)) {
                          extendedImage[i][j] = false;
                      }
                  }
              }
  ...
  public static boolean[][] thin(boolean[][] image) {
	  boolean[][] producedImage = thinningStep(image, 0);
	  int count = 0;
      while (image != producedImage) {
          image = producedImage;
          producedImage = thinningStep(image, count%2);
          count++;
      }
      return image;
  }
